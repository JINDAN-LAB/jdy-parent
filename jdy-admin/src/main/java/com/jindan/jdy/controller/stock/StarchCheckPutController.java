...
import com.jindan.jdy.service.stock.StarchCheckPutService;
...
@Slf4j
@Api(tags = "关系库-资产盘点商品关系表")
@RestController
@RequestMapping("/starchCheckPut")
public class StarchCheckPutController{

    @Reference(version = "${service.version}", check = false)
    private StarchCheckPutService stockDepositoryService;
    ...
    public ResultVo seleteJdyFlowCatalog(@ApiParam(value = "jdyFlowCatalog", required = true)
                                         @RequestBody StarchCheckPut jdyFlowCatalog){
        List<StarchCheckPut> list  = stockDepositoryService.queryCatList(jdyFlowCatalog);
    ...
    public ResultVo updateStarchMaintainRegister( @ApiParam(value = "warehouseDepository", required = true)
                                                  @RequestBody StarchCheckPut warehouseDepository){
    ...
    public ResultVo seleteCatalog(@ApiParam(name = "departmentSuggestDto", required = false)
                                  @RequestBody StarchCheckPut departmentSuggestDto){
